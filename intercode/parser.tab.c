...
{
       0,    77,    77,    88,    89,    92,    96,   103,   104,   105,
     108,   111,   118,   119,   120,   121,   122,   123,   124,   127,
     128,   131,   132,   135,   162,   189,   192,   193,   196,   197,
     198,   199,   200,   201,   202,   203,   204,   207,   245,   248,
     249,   250,   253,   254,   257,   258,   261,   262,   265,   268,
     271,   272,   276,   277,   280,   283,   286,   289,   290,   291,
...
#line 111 "parser.y" /* yacc.c:1646  */
    ...
    addVariable((yyvsp[0].treenode)->__string, (yyvsp[-1].treenode)->left->name);
}
...
#line 135 "parser.y" /* yacc.c:1646  */
    ...
    addFunction((yyvsp[-4].treenode)->__string, (yyvsp[-5].treenode)->left->name, para_list);
...
#line 162 "parser.y" /* yacc.c:1646  */
    {
        ...
        Node *para_point = (yyvsp[-3].treenode)->left;
        while(para_point)
        {
            if(!para_point->left->right) // only one parameter
            {
        ...
        addFunction((yyvsp[-5].treenode)->__string, (yyvsp[-6].treenode)->left->name, para_list);		
    }
...
#line 207 "parser.y" /* yacc.c:1646  */
    ...
    // semantic analysis
    if((yyvsp[0].treenode)->left->name == "Constant")
    {
        if(checkConstant((yyvsp[-2].treenode)->left->__string, (yyvsp[0].treenode)->left->left->name) == false)
        {
            stringstream ss;
            ss << yylineno;
            string error = "Semantic Error at line "+ss.str()+": constant type is not satisfied with "+(yyvsp[-2].treenode)->left->__string;
    ...
    else if((yyvsp[0].treenode)->left->name == "LValue")
    {
        if(checkLvalue((yyvsp[-2].treenode)->left->__string, (yyvsp[0].treenode)->left->left->__string) == false)
        {
            stringstream ss;
            ss << yylineno;
            string error = "Semantic Error at line "+ss.str()+ ": "+ (yyvsp[0].treenode)->left->left->__string+ " type is not satisfied with "+ (yyvsp[-2].treenode)->left->__string;
            // cout << "Semantic Error at line " << yylineno << ": " << $3->left->left->__string << " type is not satisfied with " << $1->left->__string << endl;
            semantic_error.push_back(error);
        }
    }
    else if((yyvsp[0].treenode)->left->name == "Call")
    {
        if(checkCall((yyvsp[-2].treenode)->left->__string, (yyvsp[0].treenode)->left->left->__string) == false)
        {
            stringstream ss;
            ss << yylineno;
            string error = "Semantic Error at line "+ss.str()+ ": "+"function " + (yyvsp[0].treenode)->left->left->__string + " type is not satisfied with " +(yyvsp[-2].treenode)->left->__string;
